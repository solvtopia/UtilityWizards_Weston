...
#End Region
    ...
    <WebMethod()> Public Function UploadFile(ByVal req As ApiRequest, ByVal fData As Byte(), ByVal fName As String) As ApiResponse
        Dim retVal As New ApiResponse(req.UseSandboxDb)
        ...
        Try
            If req.apiKey <> "" Then
                ...
                Dim filePath As String = Server.MapPath(String.Format("~/Uploads/{0}", fName))

                retVal.responseCode = Enums.ApiResultCode.success
                retVal.responseMessage = CommonCore.Shared.Common.UploadFileToAWS(filePath, fData, req.UseSandboxDb)
                retVal.responseObject = Nothing
            Else
                retVal.responseCode = Enums.ApiResultCode.failed
                retVal.responseMessage = "No API Key Provided"
